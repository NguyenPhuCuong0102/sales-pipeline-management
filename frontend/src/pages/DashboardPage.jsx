...
import { Card, Col, Row, Statistic, List, Spin, Empty, Tag, Tooltip, Button, Space } from 'antd';
import { 
...
const DashboardPage = () => {
  ...
  const data = stats || {
    ...
    revenue_by_stage: [], upcoming_deals: [], my_tasks: []
  };

  // --- STYLE CHUNG CHO CARD NỔI BẬT ---
  const cardStyle = {
    borderRadius: 8,
    boxShadow: '0 4px 12px rgba(0, 0, 0, 0.08)', // Đổ bóng đậm hơn
    border: '1px solid #f0f0f0', // Viền nhẹ
    height: '100%',
    overflow: 'hidden'
  };

  // --- 1. CÁC THẺ KPI (CÓ VIỀN MÀU TRÊN CÙNG) ---
  const KpiCards = () => (
    <Row gutter={[16, 16]}>
      <Col xs={24} sm={12} lg={6}>
        <Card bordered={false} hoverable style={{ ...cardStyle, borderTop: '4px solid #3f8600' }}>
          <Statistic 
            title={<span style={{ color: '#666', fontWeight: 600 }}>Doanh thu (Open + Won)</span>}
            value={data.expected_revenue} 
            prefix={<DollarCircleOutlined style={{ backgroundColor: '#f6ffed', padding: 8, borderRadius: '50%', color: '#3f8600' }} />} 
            suffix="đ" 
            valueStyle={{ color: '#3f8600', fontWeight: 'bold' }} 
            formatter={val => new Intl.NumberFormat('vi-VN', { compactDisplay: 'short', notation: 'compact' }).format(val)} 
          />
        </Card>
      </Col>
      <Col xs={24} sm={12} lg={6}>
        <Card bordered={false} hoverable style={{ ...cardStyle, borderTop: '4px solid #1890ff' }}>
          <Statistic 
            title={<span style={{ color: '#666', fontWeight: 600 }}>Cơ hội Đang mở</span>}
            value={data.open_deals_count} 
            prefix={<ShoppingOutlined style={{ backgroundColor: '#e6f7ff', padding: 8, borderRadius: '50%', color: '#1890ff' }} />} 
      ...
      <Col xs={24} sm={12} lg={6}>
        <Card bordered={false} hoverable style={{ ...cardStyle, borderTop: '4px solid #722ed1' }}>
          <Statistic 
            title={<span style={{ color: '#666', fontWeight: 600 }}>Khách hàng Mới (30 ngày)</span>}
            value={data.new_customers_count} 
            prefix={<UserOutlined style={{ backgroundColor: '#f9f0ff', padding: 8, borderRadius: '50%', color: '#722ed1' }} />} 
      ...
      <Col xs={24} sm={12} lg={6}>
        <Card bordered={false} hoverable style={{ ...cardStyle, borderTop: '4px solid #fa8c16' }}>
          <Statistic 
            title={<span style={{ color: '#666', fontWeight: 600 }}>Tỷ lệ Thắng</span>}
            value={data.win_rate} 
            suffix="%" 
            prefix={<TrophyOutlined style={{ backgroundColor: '#fff7e6', padding: 8, borderRadius: '50%', color: '#fa8c16' }} />} 
  ...
  const RevenueChart = () => (
    <Card 
      title={<span>💰 Giá trị Tiềm năng theo Giai đoạn</span>} 
      bordered={false} 
      style={cardStyle}
  ...
  return (
    <div>
      <h2 style={{ marginBottom: 24, fontWeight: 700, color: '#262626' }}>
        {user?.role === 'MANAGER' || user?.role === 'ADMIN' ? '📊 Bảng Điều khiển Quản lý' : `🚀 Xin chào, ${user?.username}`}
      </h2>
      ...
      <Row gutter={[24, 24]} style={{ marginTop: 24 }}>
        {user?.role === 'MANAGER' || user?.role === 'ADMIN' ? (
            ...
            <>
                ...
                <Col xs={24} lg={12}><UpcomingList /></Col>
            </>
        )}
      </Row>
    </div>
